...
class MyGRU(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        for i in range(num_layers):
            encoder, state_h = GRU(units=self.output_dim, return_sequences=True, return_state=True, dropout=en_dropouts[i], name=f'encoder_gru_{i+1}')(encoder)
        ...
        for i in range(num_layers):
            decoder = GRU(units=self.output_dim, return_sequences=True, dropout=de_dropouts[i], name=f'decoder_gru_{i+1}')(decoder, initial_state=encoder_states[i])
